...
def transactions_to_df(txns: list[dict]) -> pd.DataFrame:
    ...
    # amountInAccountCurrency: valor já convertido pra moeda da conta (BRL).
    # Necessário pra transações em moeda estrangeira (ex: assinaturas em USD),
    # onde "amount" vem na moeda original e não bate com o valor real cobrado.
    keep = [c for c in ["id", "date", "description", "amount", "amountInAccountCurrency",
                        "currencyCode", "category", "type", "accountName", "accountId"]
            if c in df.columns]
    df = df[keep]
