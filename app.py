...
# ── Flags manuais (tirar transação dos cálculos) ──────────────────────────────
# Tabela Supabase: transacao_flags
#   create table transacao_flags (
#     descricao  text not null, data date not null, valor numeric not null,
#     ocorrencia int not null default 0, motivo text not null default 'oculta',
#     nota text, criado_em timestamptz default now(),
#     primary key (descricao, data, valor, ocorrencia)
#   );
# Chave = (descricao, data, valor, ocorrencia) porque o df combinado não tem id
# estável entre Supabase e Pluggy. "ocorrencia" = cumcount dentro do grupo,
# cobre duplicatas exatas no mesmo dia. A data usada é a JÁ corrigida por
# apply_date_overrides — add_flag_columns roda depois dele.
MOTIVOS_FLAG = {"oculta":"oculta","reembolso":"reembolso manual"}
...
def load_transacao_flags() -> pd.DataFrame:
    cols = ["descricao","data","valor","ocorrencia","motivo","nota"]
    try: res = get_supabase().table("transacao_flags").select("*").execute()
    except Exception: return pd.DataFrame(columns=cols)
    if not res.data: return pd.DataFrame(columns=cols)
    d = pd.DataFrame(res.data)
    d["data"] = d["data"].astype(str).str[:10]; d["valor"] = d["valor"].astype(float)
    if "ocorrencia" not in d.columns: d["ocorrencia"] = 0
    d["ocorrencia"] = d["ocorrencia"].fillna(0).astype(int)
    if "motivo" not in d.columns: d["motivo"] = "oculta"
    if "nota" not in d.columns: d["nota"] = None
    return d

def save_transacao_flag(descricao, data_str, valor, ocorrencia, motivo="oculta", nota=None):
    row = {"descricao":descricao,"data":data_str,"valor":round(float(valor),2),
           "ocorrencia":int(ocorrencia),"motivo":motivo}
    if nota: row["nota"] = nota
    get_supabase().table("transacao_flags").upsert(row, on_conflict="descricao,data,valor,ocorrencia").execute()
    load_transacao_flags.clear()

def delete_transacao_flag(descricao, data_str, valor, ocorrencia):
    (get_supabase().table("transacao_flags").delete()
        .eq("descricao",descricao).eq("data",data_str)
        .eq("valor",round(float(valor),2)).eq("ocorrencia",int(ocorrencia)).execute())
    load_transacao_flags.clear()

def add_flag_columns(df: pd.DataFrame, flags: pd.DataFrame) -> pd.DataFrame:
    """Adiciona _ocorrencia, _flag_key, motivo_flag, ignorada, neutralizada.
    Roda no df global (antes de qualquer filtro) — o cumcount depende da
    ordenação completa pra ser estável."""
    df = df.copy()
    if df.empty:
        for c,v in [("_ocorrencia",0),("_flag_key",""),("motivo_flag",""),("ignorada",False),("neutralizada",False)]:
            df[c] = v
        return df
    _dkey = df["data"].dt.strftime("%Y-%m-%d"); _vkey = df["valor"].round(2)
    df["_ocorrencia"] = df.groupby([df["descricao"],_dkey,_vkey]).cumcount()
    df["_flag_key"] = [flag_key(a,b,c,d) for a,b,c,d in
                       zip(df["descricao"],df["data"],df["valor"],df["_ocorrencia"])]
    if flags is None or flags.empty:
        df["motivo_flag"] = ""
    else:
        mapa = {flag_key(r["descricao"],r["data"],r["valor"],r["ocorrencia"]):r["motivo"]
...
def status_txn(row) -> str:
    """Rótulo do estado da transação. Vazio = entra nos cálculos."""
    m = row.get("motivo_flag","")
    if m: return MOTIVOS_FLAG.get(m,m)
    if row.get("reembolsado",False): return "reembolso auto"
    return ""
...
# Flags manuais — depois dos date overrides (a data faz parte da chave)
...
with st.sidebar:
    ...
    if _n_flags_total:
        st.markdown(f"<p style='color:#555;font-size:0.75rem;font-family:DM Mono,monospace;margin-top:0.6rem;'>{_n_flags_total} transação(ões) fora dos cálculos (manual)</p>",unsafe_allow_html=True)
    st.divider()
...
with tab1:
    ...
    # Gastos — barra vermelha hachurrada, sobreposta (valor absoluto)
        ...
        **build_plotly_theme(), height=420, bargap=0.25, barmode="overlay",
        ...
        bar_color = "#cc1111" if tipo == "Gastos" else get_accent()
...
with tab4:
    ...
    mostrar_ocultas=st.checkbox("mostrar transações marcadas (em cinza)",value=True,key="chk_mostrar_ocultas")
    if not mostrar_ocultas: show_raw=show_raw[~show_raw["ignorada"]].copy()
    if show_raw.empty: show_raw["status"]=pd.Series(dtype=str)
    ...
    def style_row(row):
        if str(row["Status"]).strip(): return ["color: #555"]*len(row)
    ...
    if show_raw.empty:
        ...
        with col_f1:
            motivo_novo=st.radio("motivo",options=list(MOTIVOS_FLAG.keys()),format_func=lambda m:MOTIVOS_FLAG[m],horizontal=True,key="tab4_motivo_flag",label_visibility="collapsed")
...
if df_saldos.empty:
    ...
    with nw_tab2:
        ...
        df_proj_base=df[df["categoria"].isin(cats_sel)&~df["neutralizada"]].copy()
