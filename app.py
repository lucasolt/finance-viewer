...
# ── Flags manuais de transação (ocultar / reembolso manual) ───────────────────
# Tabela Supabase: transacao_flags
#
# DDL:
#   create table transacao_flags (
#     descricao   text    not null,
#     data        date    not null,
#     valor       numeric not null,
#     ocorrencia  int     not null default 0,
#     motivo      text    not null default 'oculta',
#     nota        text,
#     criado_em   timestamptz default now(),
#     primary key (descricao, data, valor, ocorrencia)
#   );
#   alter table transacao_flags enable row level security;
#   create policy "allow all" on transacao_flags
#     for all using (true) with check (true);
#
# A chave é (descricao, data, valor, ocorrencia) porque o df combinado não tem
# um id estável entre Supabase e Pluggy. "ocorrencia" é o cumcount dentro do
# grupo (descricao, data, valor) — cobre o caso de duas transações idênticas
# no mesmo dia. Se houver N idênticas, qual delas é escondida é indiferente.

MOTIVOS_FLAG = {
    "oculta":    "🙈 oculta",
    "reembolso": "🔁 reembolso manual",
}

def flag_key(descricao, data, valor, ocorrencia) -> str:
    d = data.strftime("%Y-%m-%d") if hasattr(data, "strftime") else str(data)[:10]
    return f"{descricao}||{d}||{float(valor):.2f}||{int(ocorrencia)}"

@st.cache_data(ttl=60)
def load_transacao_flags() -> pd.DataFrame:
    sb = get_supabase()
    try:
        res = sb.table("transacao_flags").select("*").execute()
    except Exception:
        return pd.DataFrame(columns=["descricao", "data", "valor", "ocorrencia", "motivo", "nota"])
    if not res.data:
        return pd.DataFrame(columns=["descricao", "data", "valor", "ocorrencia", "motivo", "nota"])
    dfl = pd.DataFrame(res.data)
    dfl["data"] = dfl["data"].astype(str).str[:10]
    dfl["valor"] = dfl["valor"].astype(float)
    if "ocorrencia" not in dfl.columns:
        dfl["ocorrencia"] = 0
    dfl["ocorrencia"] = dfl["ocorrencia"].fillna(0).astype(int)
    if "motivo" not in dfl.columns:
        dfl["motivo"] = "oculta"
    if "nota" not in dfl.columns:
        dfl["nota"] = None
    return dfl

def save_transacao_flag(descricao, data_str, valor, ocorrencia,
                        motivo: str = "oculta", nota: str = None):
    row = {
        "descricao": descricao,
        "data": data_str,
    ...
    if nota:
        row["nota"] = nota
    get_supabase().table("transacao_flags").upsert(
        row, on_conflict="descricao,data,valor,ocorrencia"
    ).execute()
    load_transacao_flags.clear()

def delete_transacao_flag(descricao, data_str, valor, ocorrencia):
    (get_supabase().table("transacao_flags")
        .delete()
        .eq("descricao", descricao)
        .eq("data", data_str)
        .eq("valor", round(float(valor), 2))
        .eq("ocorrencia", int(ocorrencia))
        .execute())
    load_transacao_flags.clear()

def add_flag_columns(df: pd.DataFrame, flags: pd.DataFrame) -> pd.DataFrame:
    """Adiciona _ocorrencia, _flag_key, motivo_flag, ignorada e neutralizada.
    Deve rodar no df global (antes de qualquer filtro) — o cumcount depende
    da ordenação completa pra ser estável."""
    df = df.copy()
    if df.empty:
        for c, v in [("_ocorrencia", 0), ("_flag_key", ""), ("motivo_flag", ""),
                     ("ignorada", False), ("neutralizada", False)]:
            df[c] = v
        return df

    _dkey = df["data"].dt.strftime("%Y-%m-%d")
    _vkey = df["valor"].round(2)
    df["_ocorrencia"] = df.groupby([df["descricao"], _dkey, _vkey]).cumcount()
    df["_flag_key"] = [
        flag_key(d, dt, v, o)
        for d, dt, v, o in zip(df["descricao"], df["data"], df["valor"], df["_ocorrencia"])
    ]
    ...
    if flags is None or flags.empty:
        ...
        mapa = {
            flag_key(r["descricao"], r["data"], r["valor"], r["ocorrencia"]): r["motivo"]
            for _, r in flags.iterrows()
        }
        df["motivo_flag"] = df["_flag_key"].map(mapa).fillna("")

    df["ignorada"] = df["motivo_flag"] != ""
    if "reembolsado" not in df.columns:
        df["reembolsado"] = False
    df["reembolsado"] = df["reembolsado"].fillna(False).astype(bool)
    df["neutralizada"] = df["reembolsado"] | df["ignorada"]
    return df

def status_txn(row) -> str:
    """Rótulo curto do estado da transação (vazio = normal/computada)."""
    m = row.get("motivo_flag", "")
    if m:
        return MOTIVOS_FLAG.get(m, f"🚫 {m}")
    if row.get("reembolsado", False):
        return "🔁 reembolso auto"
    return ""
...
# Flags manuais (ocultar / reembolso manual) — precisa rodar no df completo
df_flags = load_transacao_flags()
df = add_flag_columns(df, df_flags)
...
with st.sidebar:
    ...
    # Contador de flags manuais ativas
    _n_flags_total = int(df["ignorada"].sum())
    if _n_flags_total:
        st.markdown(
            f"<p style='color:#555;font-size:0.75rem;font-family:DM Mono,monospace;margin-top:0.6rem;'>"
            f"🙈 {_n_flags_total} transação(ões) marcada(s) manualmente</p>",
            unsafe_allow_html=True
        )
...
# dff_tabela: tudo (inclui reembolsados e ocultas) — pra tabela de transações
dff_tabela = df[mask_data & df["categoria"].isin(cats_sel)].copy()
# dff: exclui neutralizadas (reembolso auto/manual + ocultas) — gráficos e cálculos
dff = dff_tabela[~dff_tabela["neutralizada"]].copy()
if tipo == "Gastos":
    dff = dff[dff["valor"] < 0]
elif tipo == "Receitas":
    dff = dff[dff["valor"] > 0]
...
# dff_total: aplica todos os filtros exceto tipo — usado nos KPIs (exclui neutralizadas)
dff_total = dff_tabela[~dff_tabela["neutralizada"]].copy()
...
# Linha discreta: gasto/recebido/saldo do período + transações
_n_neutra_periodo = int(dff_tabela["neutralizada"].sum())
_neutra_str = (
    f"<div style='color:#555;font-size:0.78rem;font-family:DM Mono,monospace;'>"
    f"{_n_neutra_periodo} neutralizada(s)</div>" if _n_neutra_periodo else ""
)
st.markdown(
...
# Mini-painel: fatura + data de atualização
...
with tab4:
    # aplica filtro de tipo mas mantém neutralizadas visíveis
    if tipo == "Gastos":
    ...
    mostrar_ocultas = st.checkbox(
        "mostrar transações marcadas (em cinza)", value=True, key="chk_mostrar_ocultas"
    )
    if not mostrar_ocultas:
        show_raw = show_raw[~show_raw["ignorada"]].copy()

    if show_raw.empty:
        show_raw["status"] = pd.Series(dtype=str)
    else:
        show_raw["status"] = show_raw.apply(status_txn, axis=1)

    _cols_raw = ["data", "descricao", "categoria", "valor", "origem"]
    if "fonte" in show_raw.columns:
        _cols_raw.append("fonte")
    _cols_raw += ["status", "motivo_flag", "reembolsado", "_ocorrencia"]
    show_raw = (show_raw[_cols_raw]
    ...
    show_display = show_display.rename(columns={
        ...
        "valor": "Valor", "origem": "Origem", "fonte": "Fonte", "status": "Status",
    })

    def style_row(row):
        # qualquer status preenchido = transação neutralizada → cinza
    ...
    n_auto = int((show_raw["reembolsado"] & (show_raw["motivo_flag"] == "")).sum())
    n_manual = int((show_raw["motivo_flag"] != "").sum())
    if n_auto or n_manual:
        st.markdown(
            f"<p style='color:#555;font-size:0.75rem;font-family:DM Mono,monospace;'>"
            f"cinza = fora dos cálculos · {n_auto} reembolso(s) automático(s) · "
            f"{n_manual} marcação(ões) manual(is)</p>",
            unsafe_allow_html=True
        )

    st.download_button(
        "⬇ baixar transações (.xlsx)",
        df_to_xlsx(show_raw.drop(columns=["_ocorrencia"]).rename(columns={
            "data": "Data", "descricao": "Descrição", "categoria": "Categoria",
            "valor": "Valor", "origem": "Origem", "fonte": "Fonte",
            "status": "Status", "motivo_flag": "Motivo", "reembolsado": "Reembolso auto"})),
        "transacoes.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
    )

    # ── Editar transação (categoria + flags manuais) ──────────────────────────
    st.markdown("---")
    ...
    if show_raw.empty:
        ...
        # Rótulo legível e único (prefixo com índice evita colisão de labels)
        def _row_label(i, row):
            ...
            return f"#{i} · {data_str} · {desc} · {val} · [{cat}]{st_}"
        ...
        valor_sel  = float(sel_row["valor"])
        ocorr_sel  = int(sel_row["_ocorrencia"])
        cat_atual  = sel_row["categoria"]
        motivo_sel = sel_row["motivo_flag"]
        ...
        # --- flags manuais (ocultar / reembolso) ---
        st.markdown(
            "<p style='color:#555;font-size:0.75rem;font-family:DM Mono,monospace;margin-top:0.8rem;'>"
            "tirar dos cálculos</p>", unsafe_allow_html=True
        )
        col_f1, col_f2, col_f3 = st.columns([2, 1, 1])
        with col_f1:
            motivo_novo = st.radio(
                "motivo", options=list(MOTIVOS_FLAG.keys()),
                format_func=lambda m: MOTIVOS_FLAG[m],
                horizontal=True, key="tab4_motivo_flag",
                label_visibility="collapsed",
            )
        with col_f2:
            btn_flag = st.button("🚫 tirar dos cálculos", use_container_width=True,
                                 key="btn_flag_txn")
        with col_f3:
            btn_unflag = st.button("↩️ restaurar", use_container_width=True,
                                   key="btn_unflag_txn", disabled=(motivo_sel == ""))

        if btn_flag:
            try:
                save_transacao_flag(desc_sel, data_sel, valor_sel, ocorr_sel, motivo_novo)
                st.success(f"Transação marcada como **{MOTIVOS_FLAG[motivo_novo]}** e removida dos cálculos.")
                st.rerun()
            except Exception as _e:
                st.error(f"Erro ao marcar: {_e}")

        if btn_unflag:
            try:
                delete_transacao_flag(desc_sel, data_sel, valor_sel, ocorr_sel)
                st.success("Transação restaurada aos cálculos.")
                st.rerun()
            except Exception as _e:
                st.error(f"Erro ao restaurar: {_e}")

        # --- marcar par de reembolso (2+ de uma vez) ---
        with st.expander("🔁 marcar par de reembolso (várias de uma vez)", expanded=False):
            ...
            motivo_par = st.radio(
                "motivo do par", options=list(MOTIVOS_FLAG.keys()),
                format_func=lambda m: MOTIVOS_FLAG[m],
                horizontal=True, index=1, key="tab4_motivo_par",
                label_visibility="collapsed",
            )
            if par_labels:
                ...
                st.markdown(
                    f"<p style='color:#555;font-size:0.78rem;font-family:DM Mono,monospace;'>"
                    f"soma da seleção: <span style='color:{_cor}'>{fmt_brl_signed(_soma)}</span>"
                    f"{' ✓ zera' if abs(_soma) < 0.01 else ''}</p>",
                    unsafe_allow_html=True
                )
            if st.button("🚫 marcar seleção", key="btn_flag_par",
                         disabled=(len(par_labels) == 0)):
                erros = []
                for lbl in par_labels:
                    r = show_raw.iloc[opcoes_map[lbl]]
                    try:
                        save_transacao_flag(
                            r["descricao"], r["data"].strftime("%Y-%m-%d"),
                            float(r["valor"]), int(r["_ocorrencia"]), motivo_par,
                        )
                    except Exception as _e:
                        erros.append(f"{lbl}: {_e}")
                if erros:
                    st.error("Erros:\n" + "\n".join(erros))
                else:
                    st.success(f"{len(par_labels)} transação(ões) removida(s) dos cálculos.")
                    st.rerun()

        # --- gerenciar todas as flags ativas ---
                ...
                for _i, _fl in df_flags.sort_values("data", ascending=False).reset_index(drop=True).iterrows():
                    ...
                    with cA:
                        st.markdown(
                            ...
                            f"{_fl['data']} · {str(_fl['descricao'])[:45]} · "
                            f"{fmt_brl_signed(_fl['valor'])} · "
                            f"<b style='color:#c8f060'>{MOTIVOS_FLAG.get(_fl['motivo'], _fl['motivo'])}</b>"
                            f"</span>",
                            unsafe_allow_html=True
                        )
                    with cB:
                        if st.button("↩️", key=f"unflag_{_i}", help="restaurar aos cálculos"):
                            try:
                                delete_transacao_flag(
                                    _fl["descricao"], _fl["data"],
                                    _fl["valor"], _fl["ocorrencia"],
                                )
                                st.rerun()
                            except Exception as _e:
                                st.error(f"Erro: {_e}")

        # Info sobre overrides de categoria ativos
        ...
        with st.expander("🔍 debug", expanded=False):
            ...
            hits = df[df["descricao"] == desc_sel][["data","descricao","valor","categoria","_ocorrencia","motivo_flag"]].head(10)
